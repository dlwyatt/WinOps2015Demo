...
.DESCRIPTION
Takes all of the given arguments and constructs a DscResourceProperty object to be used by New-cDscResource.
...
.EXAMPLE
C:\PS> New-cDscResourceProperty -Name "Ensure" -Type "String" -Attribute Write -ValidateSet "Present","Absent" -Description "Ensure Present or Absent"
Name        : Ensure
Type        : String
Attribute   : Write
...
#>
function New-cDscResourceProperty
{
    [CmdletBinding()]
...
.EXAMPLE
C:\PS> New-cDscResource -Name "UserResource" -Property $UserName,$Ensure,$Password -Path "$pshome\Modules\UserResource" -ClassVersion 1.0 -FriendlyName "User" -Force
#>
function New-cDscResource
{
    ...
    if (-not (Test-cDscResource $fullPath))
    {
...
.EXAMPLE
C:\PS> Update-cDscResource -ResourceDirectory "UserResource" -Property $UserName,$Ensure,$Password -ClassVersion 1.0 -FriendlyName "User" -Force
#>
function Update-cDscResource
{
...
.EXAMPLE
C:\PS> Test-cDscResource "MSFT_UserResource"
True
#>
function Test-cDscResource
{
    [CmdletBinding()]
    [OutputType([Boolean])]
    param
...
function Test-cDscSchemaEncoding
{
...
function Test-cDscSchema
{
    ...
    {
        $Errors = 0
        Test-cDscSchemaInternal -Schema $Path -errorIdsRef [ref]$Errors
    }
    else
    {
        Test-cDscSchemaInternal -Schema $Path
    }
}

# Tests a given schema file to make sure it passes all contracts required by Dsc.
function Test-cDscSchemaInternal
{
    [OutputType([Boolean])]
    param
    (
    ...
    if (-not (Test-cDscSchemaEncoding $Schema))
    {
        return $false;
    }
...
#Given a schema file, if the schema file passes Test-cDscSchema
...
function Import-cDscSchema
{
    ...
    # Passed by reference to Test-cDscSchema
    $cimClass = 0

    if (-not (Test-cDscSchemaInternal $Schema ([ref]$cimClass)))
    {
        #If the file does not pass Test-cDscSchema, return nothing.
        return
    }

    Write-Verbose ($localizedData['ImportTestSchemaVerbose'])

    #If the file passes Test-cDscSchema
