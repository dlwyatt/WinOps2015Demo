...
Import-LocalizedData LocalizedData -FileName PSHOrg_cServiceResource.strings.psd1
...
function Get-TargetResource
{
    ...
    if ($svc -and $svcWmi)
    {
        $configuration['StartupType']  = (NormalizeStartupType $svcWmi.StartMode).ToString()
        $configuration['State']        = $svc.Status.ToString()
        $configuration['Path']         = $svcWmi.PathName
        $configuration['DisplayName']  = $svc.DisplayName
        $configuration['Description']  = $svcWmi.Description
        $configuration['Dependencies'] = [string[]](@() + ($svc.ServicesDependedOn | %{$_.Name}))

        $configuration['BuiltInAccount'] = if($svcWmi.StartName -ieq "LocalSystem") {"LocalSystem"} `
...
function Test-TargetResource
{
    ...
    if (-not $BuiltInAccount -and -not $Credential)
    {
    ...
    if ($Ensure -eq 'Absent')
    {
        return $null -eq $svc -and $null -eq $svcWmi
    }
    else
    {
        ...
        if($BuiltInAccount) {$null=$getUserNameAndPasswordArgs.Add("BuiltInAccount",$BuiltInAccount)}
        if($Credential) {$null=$getUserNameAndPasswordArgs.Add("Credential",$Credential)}
    ...
    else
    {
        ...
        if ($null -eq $svc)
        {
            ...
            try
            {
                ...
                if ($null -eq $svc) { throw 'New-Service was successful, but GetServiceResource still returned null.' }
            }
            catch
            {
...
function GetWMIService
{
    ...
    Get-WmiObject Win32_Service -Filter "Name = '$Name'" -ErrorAction Ignore
}
...
function WriteCredentialProperties
{
    ...
    if($userName -ne $null -and !(TestUserName $SvcWmi $userName) -and $PSCmdlet.ShouldProcess($SvcWmi.Name,$LocalizedData.SetCredentialWhatIf))
    {
        ...
        StopService (GetServiceResource $SvcWmi.Name)
    }
}
...
<#
.Synopsis
Gets a service corresponding to a name
#>
function GetServiceResource
{
    ...
    Get-Service $Name -ErrorAction Ignore
}
